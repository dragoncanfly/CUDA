...
/* Background
   ...
   2) Calculate the new pixel value:
	...
	recap:
	i_{k+1} = (A+B+C)/D
	A: sum of i_k's neighbors.在sum中不包含i_k,
	B: sum of t's neighbors,t是目标图像中对应的像素值，不包含t
	C: sum of difference between s and its neighbors，s是源图像中对应的像素值
	D: number of neighbors that i_k has,如果i_k是位于边界上此值小于4
   */
...
//check whether a value is within the image bounds
...
__global__ void maskPredicateKernel(const uchar4* const d_sourceImg, int* d_borderPredicate, int* d_interiorPredicate,
									const size_t numRowsSource, const size_t numColsSource)
{
	...
	if (!withinBounds(p.x, p.y, numRowsSource, numColsSource))
		return;
	//return through each pixel and determine if its on the border, or if its on the interior border
	if (masked(d_sourceImg[m]))
	{
		int inbounds = 0;
		int interior = 0;
		//count how many of our neighbors are masked, and how many neighbors we have
		...
		//clear out the values so we don't have to memset this destination stuff
...
__global__ void separateChannelsKernel(const uchar4* const inputImageRGBA, float* const redChannel, float* const greenChannel, float* const blueChannel, size_t numRows, size_t numCols)
{
	const int2 p = get2dPos();
...
__global__ void recombineChannelsKernel(uchar4* outputImageRGBA, float* const redChannel, float* const greenChannel, float* const blueChannel, size_t numRows, size_t numCols)
{
	const int2 p = get2dPos();
	const int m = getm(p.x, p.y, numCols);
	if (!withinBounds(p.x, p.y, numRows, numCols))
...
__global__ void jacobiKernel(float* d_in, float* d_out, const int* d_borderPredicate, const int* d_interiorPredicate, float* d_source, float* d_dest, size_t numRows, size_t numCols)
{
	const int2 p = get2dPos();
	const int m = getm(p.x, p.y, numCols);
	if (!withinBounds(p.x, p.y, numRows, numCols))
                ...
                uchar4* const h_blendedImg) //OUT
{
  ...
  */
	...
	maskPredicateKernel << <gridSize, blockSize >> > (d_sourceImg, d_borderPredicate, d_interiorPredicate, numRowsSource, numColsSource);
	...
	separateChannelsKernel << <gridSize, blockSize >> > (d_sourceImg, d_sourceImgR, d_sourceImgG, d_sourceImgB, numRowsSource, numColsSource);
	cudaDeviceSynchronize();
	checkCudaErrors(cudaGetLastError());

	separateChannelsKernel << <gridSize, blockSize >> > (d_destImg, d_destImgR, d_destImgG, d_destImgB, numRowsSource, numColsSource);
	cudaDeviceSynchronize();
	checkCudaErrors(cudaGetLastError());
	...
	for (int i = 0; i < 800; i++)
	{
		jacobiKernel << <gridSize, blockSize >> > (d_r0, d_r1, d_borderPredicate, d_interiorPredicate, d_sourceImgR, d_destImgR, numRowsSource, numColsSource);
		std::swap(d_r0, d_r1);
		jacobiKernel << <gridSize, blockSize >> > (d_g0, d_g1, d_borderPredicate, d_interiorPredicate, d_sourceImgG, d_destImgG, numRowsSource, numColsSource);
		std::swap(d_g0, d_g1);
		jacobiKernel << <gridSize, blockSize >> > (d_b0, d_b1, d_borderPredicate, d_interiorPredicate, d_sourceImgB, d_destImgB, numRowsSource, numColsSource);
		std::swap(d_b0, d_b1);
	}

	//6) 
	//lets assume that d_r0, d_g0, d_b0 are the final pass
	recombineChannelsKernel << <gridSize, blockSize >> > (d_finalImg, d_r0, d_g0, d_b0, numRowsSource, numColsSource);
	cudaDeviceSynchronize();
	checkCudaErrors(cudaGetLastError());
